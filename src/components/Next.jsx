...
const Next = () => {
    ...
    return (
        <div>
            <h2 className="mt-16 font-sans font-semibold text-5xl text-center text-darkGreen">What to read next</h2>

            <div className="mt-10 mx-60 gap-x-8 grid grid-cols-1 md:grid-cols-3">
                {articles.map(article => (
                    <div key={article.id} className="flex flex-col items-center mb-10">
                        ...
                        <Link to={article.link}>
                            ...
                            <h3 className="xl:mx-20 font-sans text-center xs:text-xl xs:font-semibold font-bold hover:underline">{article.title}</h3>
                        </Link>
                    </div>
                ))}
            </div>
